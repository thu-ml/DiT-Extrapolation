...
def _attn_fwd_inner(acc, l_i, m_i, q, q_scale, kv_len,
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        m_ij = t.maximum(m_i, tl.max(qk, 1))
        qk = qk - m_ij[:, None]
        p = tl.math.exp2(qk)
        l_ij = tl.sum(p, 1)
...
@triton.jit
def _attn_fwd_inner_decay(acc, l_i, m_i, q, q_scale, kv_len, 
                    K_ptrs, K_scale_ptr, V_ptrs, stride_kn, stride_vn, 
...
@triton.jit
def _attn_fwd_decay(Q, K, V, Q_scale, K_scale, Out, 
              Decay_mask,
              stride_qz, stride_qh, stride_qn,
              stride_kz, stride_kh, stride_kn,  
              stride_vz, stride_vh, stride_vn,  
    ...
    q = tl.load(Q_ptrs, mask = offs_m[:, None] < qo_len)
    q_scale = tl.load(Q_scale_ptr)
    acc, l_i = _attn_fwd_inner_decay(acc, l_i, m_i, q, q_scale, kv_len, K_ptrs, K_scale_ptr, V_ptrs, 
                                stride_kn, stride_vn,
                                Decay_mask_ptrs, stride_dmz, stride_dmh, stride_dm, stride_dn,
                                start_m,  
                                BLOCK_M, HEAD_DIM, BLOCK_N,  
...
def forward(q, k, v, q_scale, k_scale, tensor_layout="HND", output_dtype=torch.float16,    
              frame_tokens: tl.constexpr = 2040,
              BLOCK_M: tl.constexpr = 128,
              BLOCK_N: tl.constexpr = 128,
              attention_operator: Optional[dict] = None,
            ):
    ...
    if attention_operator.get("attn_func","standard") == "attention_decay":  # attention method
        decay_mask = attention_operator.get("decay_mask",None )
        if decay_mask is None:
            decay_mask = torch.zeros((b, h_qo, (qo_len + BLOCK_M - 1) // BLOCK_M, (qo_len + BLOCK_N - 1) // BLOCK_N), dtype=q.dtype, device=q.device)
    
        stride_dmz, stride_dmh, stride_dm, stride_dn = decay_mask.stride()
        _attn_fwd_decay[grid](
