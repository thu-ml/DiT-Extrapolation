...
from jintao import sage_attn,search
...
class FluxSingleAttnProcessor2_0:
    ...
    def token_merge(self, x):
        x_text = x[:, :, :512]
        x = x[:, :, 512:]
        batch_szie, head_num, toekn_num, head_dim = x.shape
        merge_ratio = int(math.sqrt(toekn_num) // 64)
        avg_pool = nn.AvgPool2d(kernel_size=merge_ratio, stride=merge_ratio)
        x = x.transpose(2, 3).reshape(batch_szie, head_dim * head_num, int(math.sqrt(toekn_num)), int(math.sqrt(toekn_num)))
        x = avg_pool(x)
        x = x.reshape(batch_szie, head_num, head_dim, toekn_num // merge_ratio**2).transpose(2, 3)
        x = torch.cat([x_text, x], 2)
        return x
    ...
    def __call__(
        ...
        # query batch for limited VRAM
            ...
            if attention_operator.get('save_one_timestep',True)== True:
                if  self.timestep.item() >= 1000.0:           
                    q_pad,k_pad,v_pad = pad_qkv(query),pad_qkv(key),pad_qkv(value)
                    _,m,block_avg= search.flash_attention_v2(q_pad,k_pad,v_pad)
                    path = f"{self.method['save_path']}/onestep_singleblock_block_{self.idx}_timestep_1.pt"
                    torch.save(block_avg,path)
            
            else:
                q_pad,k_pad,v_pad = pad_qkv(query),pad_qkv(key),pad_qkv(value)
                _,m,block_avg= search.flash_attention_v2(q_pad,k_pad,v_pad)
                path = f"{self.method['save_path']}/singleblock_block_{self.idx}_timestep_{int(self.timestep.item())}.pt"
                torch.save(block_avg,path)
            ...
            if attention_operator.get('save_one_timestep',True)== True :
                path = f"{self.method['save_path']}/onestep_singleblock_block_{self.idx}_timestep_1.pt"
                block_avg = torch.load(path,map_location="cpu").to(query.device)
                if attention_operator.get("use_decay_mask",False ) :
                    ...
                    if decay_type=="continuous":
                        ...
                        if block_entropy.isnan().any():
                            ...
                            block_entropy= -block_avg_proportion*torch.log(block_avg_proportion)
                            block_entropy = block_entropy.sum(dim=-1)
                        # assert block_entropy.isnan().any()==False, "block entropy is nan"
                        if block_entropy.isnan().any():
                        ...
                        decay_mask= get_continuous_decay(block_entropy,h_min,h_max,lambda_max,lambda_min,func_type,gamma)
                        attention_operator["decay_mask"] = decay_mask
        
                hidden_states = sage_attn(query, key, value,attention_operator=attention_operator)
        ...
        # query batch for limited VRAM
            ...
            if attention_operator.get('save_one_timestep',True)== True:
                if  self.timestep.item() >= 1000.0:           
                    q_pad,k_pad,v_pad = pad_qkv(query),pad_qkv(key),pad_qkv(value)
                    _,m,block_avg= search.flash_attention_v2(q_pad,k_pad,v_pad)
                    path = f"{self.method['save_path']}/onestep_transformerblock_block_{self.idx}_timestep_1.pt"
                    torch.save(block_avg,path)
            
            else:
                q_pad,k_pad,v_pad = pad_qkv(query),pad_qkv(key),pad_qkv(value)
                _,m,block_avg= search.flash_attention_v2(q_pad,k_pad,v_pad)
                path = f"{self.method['save_path']}/transformerblock_block_{self.idx}_timestep_{int(self.timestep.item())}.pt"
                torch.save(block_avg,path)
